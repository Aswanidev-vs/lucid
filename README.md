...
- **Fast & Lightweight**: Built with Go and PostgreSQL

### Security Features
- **XSS Protection**: HTML sanitization using bluemonday library
...
Choose **Web Service** when creating a new service on Render. This is a Go server that listens for HTTP requests — not a Static Site or Background Worker.
...
> **Note**: On the free plan, the backend goes to sleep after 15 minutes of inactivity. The first request after idle will be slow (~30 seconds wake-up time).
...
- Fonts from [Google Fonts](https://googlefonts.com/)
